...
namespace ZeroMod {
    ...
    {
        float px, py, pz, pw; // TEXCOORD0 float4
        float u, v;           // TEXCOORD1 float2
    };
    ...
    struct d3d9_video_struct {
     ...
     //   d3d9_slang_pass slang_pass0;
      //bool slang_pass0_compiled;
        ...
        {
            IDirect3DTexture9* src_tex;   // shader input (your srv)
            IDirect3DSurface9* dst_rtv;   // render target surface (where we draw)
            UINT               dst_w;
            UINT               dst_h;
