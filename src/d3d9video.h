...
namespace ZeroMod {
    ...
    struct zm_slang_vertex {
        float x, y, z, w;  // TEXCOORD0 (Position)
        float u, v;        // TEXCOORD1 (TexCoord)
    };
    ...
    typedef struct zm_zero_stage_rt {
    ...
    struct d3d9_video_struct {
        ...
        // Per-frame context for slang application (fed by draw_ss)
        struct d3d9_slang_frame_ctx
        {
            IDirect3DTexture9* src_tex;   // shader input (srv)
            IDirect3DSurface9* dst_rtv;   // render target surface (draw surface)
            UINT               dst_w;
            UINT               dst_h;

            // viewport (already in d3d9->final_viewport but keep explicit)
            UINT               vp_x;
            UINT               vp_y;

            UINT64             frame_count;
            float              frame_direction; // usually 1.0f
        };

        d3d9_slang_frame_ctx slang_frame;
        bool slang_frame_valid;
        ...
        zm_zero_stage_rt zero_pre = { nullptr, nullptr, 0, 0 };
        zm_zero_stage_rt zero_out = { nullptr, nullptr, 0, 0 };
...
typedef ZeroMod::d3d9_video_struct ZeroMod_d3d9_video_t;
