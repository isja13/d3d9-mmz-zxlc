...
namespace ZeroMod {
    ...
    bool d3d9_gfx_frame(d3d9_video_struct* d3d9, IDirect3DTexture9* texture, UINT64 frame_count)
    {
        ...
        {
            OutputDebugStringA("[ZeroMod] d3d9_gfx_frame: librashader reload pending\n");

            bool ok = ZeroMod::librashader_d3d9_load_preset(d3d9, d3d9->shader_path);
            if (!ok)
                OutputDebugStringA("[ZeroMod] d3d9_gfx_frame: librashader preset load failed\n");
        }
        d3d9->shader_reload_pending = false;
    ...
    bool d3d9_gfx_set_shader(d3d9_video_struct* d3d9, const char* shader_source)
    {
        ...
        if (zm_looks_like_path(shader_source) && !zm_looks_like_hlsl_source(shader_source)) {
            ...
            OutputDebugStringA("[ZeroMod] d3d9_gfx_set_shader: queued preset reload (librashader)\n");
            return true;
        }
