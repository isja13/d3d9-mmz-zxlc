...
namespace ZeroMod {
    ...
    bool d3d9_gfx_frame(d3d9_video_struct* d3d9, IDirect3DTexture9* texture, UINT64 frame_count)
    {
        ...
        //  only if path-based and non-empty.
        if (d3d9->shader_is_path && d3d9->shader_path && *d3d9->shader_path)
        {
            OutputDebugStringA("[ZeroMod] d3d9_gfx_frame: slang preset reload pending\n");

            // Parse-only for now (no runtime compilation/binds yet)
            ok = ZeroMod::slang_d3d9_load_preset_parse_only(d3d9);

            if (!ok)
                OutputDebugStringA("[ZeroMod] d3d9_gfx_frame: slang preset parse failed\n");
        }

        // IMPORTANT: do not let any failure keep this set, or it will hard-loop reload attempts.
        d3d9->shader_reload_pending = false;
    ...
    // Centralized setter, don't leak. keep flags consistent.
    ...
    static void zm_set_shader_path(ZeroMod::d3d9_video_struct* d3d9, const char* path)
    {
        ...
        // Only mark reload if actually changed
        bool changed = true;
    ...
    bool d3d9_gfx_set_shader(d3d9_video_struct* d3d9, const char* shader_source)
    {
        ...
        // always treat as a filesystem path to a preset
