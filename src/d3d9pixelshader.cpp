    ...
    : impl(new Impl(inner, bytecode_hash, bytecode_length, source))
{
    ...
    // (delete this line if it exists anywhere; it is wrong/no-op and harmful conceptually)
    // inner = this;
}
...
HRESULT STDMETHODCALLTYPE MyID3D9PixelShader::QueryInterface(REFIID riid, void** ppvObj)
{
    ...
    *ppvObj = nullptr;
    ...
    // Forward anything else to inner (rare but safe)
...
ULONG STDMETHODCALLTYPE MyID3D9PixelShader::AddRef()
{
   ...
   // _snprintf(b, sizeof(b),
    //    "[ZeroMod][PS WRAP] AddRef this=%p inner=%p rc=%ld\n",
    //    (void*)this, (void*)(impl ? impl->inner : nullptr), rc);
...
ULONG STDMETHODCALLTYPE MyID3D9PixelShader::Release()
{
   ...
   // char b[256];
   // _snprintf(b, sizeof(b),
        //"[ZeroMod][PS WRAP] Release this=%p inner=%p rc=%ld\n",
       // (void*)this, (void*)(impl ? impl->inner : nullptr), rc);
  //  OutputDebugStringA(b);
