...
// NOTE: filter_state.rtv_tex is a WRAPPER (MyIDirect3DTexture9*), so compare inner texture.
        ...
        auto draw_ss = [&](
            ...
            {
                {
                    // Snapshot the REAL device viewport too (and log HRESULT!)
                 //   D3DVIEWPORT9 dev_vp{};
                 //   HRESULT hr_gvp = inner->GetViewport(&dev_vp);
                }
                IDirect3DSurface9* dst = saved_rt0; // outer-scope captured real RT0
                if (!dst) return false;

                // Instead of using render_vp when cached_tex == null:
                D3DSURFACE_DESC rd{};
                dst->GetDesc(&rd);
                ...
                vp.x = 0;
                vp.y = 0;
                vp.width = (int)rd.Width;
                vp.height = (int)rd.Height;
                vp.full_width = (int)rd.Width;
                vp.full_height = (int)rd.Height;

                d3d9_update_viewport(d3d9, dst, &vp);

                D3DSURFACE_DESC desc{};
                if (FAILED(dst->GetDesc(&desc)) || !desc.Width || !desc.Height)
                    return false;
                ...
                UINT vp_x = 0;
                UINT vp_y = 0;
                UINT vp_w = (UINT)rd.Width;
                UINT vp_h = (UINT)rd.Height;

                char b[256];
                _snprintf(b, sizeof(b),
                    "[ZeroMod] LIBRA INPUT: src=%p %ux%u fmt=%u | dst_rtv=%p %ux%u fmt=%u\n",
                    (void*)src_tex, (unsigned)sd.Width, (unsigned)sd.Height, (unsigned)sd.Format,
                    (void*)dst, (unsigned)rd.Width, (unsigned)rd.Height, (unsigned)rd.Format);
                OutputDebugStringA(b);

                DBGF("LIBRA dst: %ux%u fmt=%u", rd.Width, rd.Height, rd.Format);
                DBGF("LIBRA vp: x=%u y=%u w=%u h=%u", vp_x, vp_y, vp_w, vp_h);

                if (vp_w == 0 || vp_h == 0)
                {
                    OutputDebugStringA("[ZeroMod][SLANGQ] ABORT: zero-sized VP\n");
                    return false;
                }
             //   if (!render_orig_width || !render_orig_height)
                 //   return false;
                // Queue job to run later (EndScene/Present), using EXACT SAME vp_* identifiers
                this->queue_slang_job(d3d9, src_tex, dst, vp_x, vp_y, vp_w, vp_h, this->frame_count);
...
HRESULT STDMETHODCALLTYPE MyID3D9Device::Present(
    ...
    // ---- Queued librashader job (run here, outside the scene) ----
    if (impl && impl->inner && impl->pending_slang.armed)
    {
        auto& j = impl->pending_slang;
        ...
        if (job_ok)
        {
            ...
            bool ok = ZeroMod::librashader_d3d9_frame(
                j.d3d9,
                j.src_tex,
                j.dst_rtv,
                j.vp_x,
            ...
            OutputDebugStringA(ok
                ? "[ZeroMod][SLANGQ] Present queued slang OK\n"
                : "[ZeroMod][SLANGQ] Present queued slang FAILED\n");

            if (sb) { sb->Apply(); sb->Release(); sb = nullptr; }
        }
        else
        {
            OutputDebugStringA("[ZeroMod][SLANGQ] Present: queued job invalid (missing refs or vp)\n");
        }

        // Release + clear queued job unconditionally (avoid leaks / repeats)
        if (j.src_tex) { j.src_tex->Release(); j.src_tex = nullptr; }
        if (j.dst_rtv) { j.dst_rtv->Release(); j.dst_rtv = nullptr; }
