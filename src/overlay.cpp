...
class Overlay::Impl {
    ...
    void cleanup_render_target() {
        if (imgui_context) {
            ImGui::SetCurrentContext(imgui_context);  // CRITICAL
    ...
    void set_display(D3DPRESENT_PARAMETERS* pp, MyID3D9Device* dev)
    {
        ...
        // If this crashes, inner pointer is not valid.
        inner->AddRef();
        inner->Release();
    ...
    void reset_display() {
        ...
        if (pDevice) {
            pDevice->Release();
            pDevice = nullptr;
            OutputDebugStringA("[ZeroMod] Overlay::reset_display: pDevice released\n");
        }

        hwnd = NULL;
        OutputDebugStringA("[ZeroMod] Overlay::reset_display EXIT\n");
    }
...
void Overlay::set_log_message(const std::string& message) {
    impl->begin_text();
    impl->push_text_base(std::string(message));
    impl->end_text();
}
