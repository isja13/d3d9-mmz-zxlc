...
namespace {
    ...
    static HRESULT STDMETHODCALLTYPE HookedCreateDevice(
        ...
        cg_load_init_once(g_hThisModule, realDev);
        printf("[ZeroMod] Cg: %s (%s)\n",
            cg_load_is_ready() ? "ready" : "NOT ready",
            cg_load_last_error());
        if (cg_load_is_ready()) {
            cg_runtime_init_once();
            printf("[ZeroMod] cg_runtime init: %s\n", cg_runtime_last_error());
